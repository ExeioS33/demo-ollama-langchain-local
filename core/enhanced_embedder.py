...
class EnhancedEmbedder:
    ...
    def embed_text_and_image(self, text: str, image: Image.Image) -> np.ndarray:
        ...
        L'embedding résultant utilise une pondération adaptative entre texte et image,
        avec une préférence pour le texte lorsqu'on utilise LLaVA.
        ...
        # Déterminer le poids optimal pour le texte
        # Si LLaVA est utilisé comme modèle génératif, augmenter le poids du texte
        # car LLaVA a ses propres capacités d'analyse d'image
        llm_model = os.environ.get("LLM_MODEL", "").lower()
        text_weight = 0.7 if "llava" in llm_model else 0.5

        # Combiner les embeddings avec pondération adaptative
    ...
    def _validate_embeddings(self, embeddings: np.ndarray):
        """
        Valide que les embeddings sont correctement normalisés.

        Args:
            embeddings (np.ndarray): Embeddings à valider

        Raises:
            ValueError: Si les embeddings ne sont pas correctement normalisés
        """
        ...
        if not np.allclose(norms, 1.0, atol=1e-6):
            print(
                f"Attention: Certains embeddings ne sont pas correctement normalisés. Norms: {norms}"
            )
            # Normaliser automatiquement plutôt que de lever une exception
            return False
        return True
